...
function getVersion() {
    return fetch('https://api.github.com/repos/AceSilentKill/PC7-Client/releases/latest')
        .then(res => res.json())
        .then(res => { res.name })
        .catch((e) => { console.log(e); console.log('Unable to fetch latest version, defaulting to 6.4.0...'); });
}
getVersion()
    .then((ver) => { window.latestVersion = ver })
    .finally(() => { if (window.latestVersion === undefined) window.latestVersion = '6.4.0'; });
